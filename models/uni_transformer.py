...
class BASE(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr=None, batch=None, subcomplex_pad=None):
        ...
        if self.with_sg:
            ...
            score = self.score_layer(torch.cat([x, subcomplex_pad], dim=-1), edge_index).squeeze()#【890】
        else:
            score = self.score_layer(x, edge_index).squeeze()#[890]

        perm = topk(score, self.ratio, batch)#[446]据 score 的分数，从每个批次中选出前 self.ratio 比例的最高得分项，并返回它们的索引或值。
        x = x[perm] * self.non_linearity(score[perm]).view(-1, 1)#这个操作的目的可能是根据分数权重动态调整 x 中的值，进而增强模型的表现力或加权调整输入数据。[446,128]
        batch = batch[perm]
        edge_index, edge_attr = filter_adj(#过滤图的邻接信息，保留根据 perm（即之前根据某些条件选出来的顶点或边）筛选出的边，并更新对应的边属性。
            edge_index, edge_attr, perm, num_nodes=score.size(0))#得到过滤后的边索引和边属性
...
class DualAttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def forward(self, h, h_sub, x, x_sub, edge_attr, edge_attr_sub, edge_index, edge_index_sub, mask_ligand, perm_sub, e_w=None, e_w_sub=None, fix_x=False):
        ...
        for i in range(self.num_x2h):
            ...
            h_in_sub = h_out_sub#[446,128]

        h_in_sub_pad = torch.zeros_like(h_in).to(h_in.device)
        h_in_sub_pad[perm_sub] = h_in_sub
        h_in = self.graph_fusion(torch.cat([h_in, h_in_sub_pad], dim=-1))#[890,128]
    ...
    def forward(self, h, x, mask_ligand, batch, return_all=False, fix_x=False):
        ...
        for b_idx in range(self.num_blocks):
            ...
            for l_idx, layer in enumerate(self.base_block):
                ...
                x_sub = x[perm_sub]
                edge_type_sub = self._build_edge_type(edge_index_sub, mask_ligand_sub)#构建亚复结构的边类型

                if l_idx == 0:
                    int_node = scatter_sum(h_sub, batch_sub, dim=0)#[446,128] [446]得到【2，128】根据 batch_sub 中的索引，将 h_sub 中同一批次的节点特征求和
                    ...
                    a_sub = self.W_a_main[l_idx - 1](h_sub)#线性层 128到128[446,128]
                    a_int = self.W_a_int[l_idx - 1](int_node)#同上[2,128]
                    a = self.W_bmm[l_idx - 1](a_sub * a_int.index_select(0, batch_sub))#[446,1]
                    attn = scatter_softmax(a.view(-1), batch_sub).view(-1, 1)#[446,1]
                    m_main_to_int = scatter_sum(attn * self.W_main[l_idx - 1](h_sub), batch_sub, dim=0)#[2,128]
                    main_to_int = self.W_main_to_int[l_idx - 1](m_main_to_int)#[2,128]

                    int_self = self.W_int[l_idx - 1](int_node)#[2,128]
                    z_int = torch.sigmoid(self.W_zi1[l_idx - 1](int_self) + self.W_zi2[l_idx - 1](main_to_int))#[2,128]
                    hidden_int = (1 - z_int) * int_self + z_int * main_to_int
                    int_node = self.GRU_int(hidden_int, int_node)#[2,128]
                ...
                subcomplex_pad[perm_sub] = h_sub#[890,128]

                subcomplex_pad = self.scmlp[l_idx](subcomplex_pad)#线性层 128到128
#这段代码实现了一个多层次的图神经网络框架，主要步骤包括：
# 节点与整合节点特征的交互。
# 通过注意力机制加权节点特征。
# 通过 GRU 更新整合节点特征。
# 最终将整合节点的特征反馈到图的节点中，并继续通过图神经网络层进行传播。这种多级交互可以用于复杂图结构的特征提取与学习。
            all_x.append(x)
            all_h.append(h)
